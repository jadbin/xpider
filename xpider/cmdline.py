...
def _print_commands():
    print("usage: xpider <command> [options] [args]\n")
    print("available commands:")
    cmds = _get_commands_from_module()
    for cmdname, cmdclass in sorted(cmds.items()):
        print("  {:<10} {}".format(cmdname, cmdclass.short_desc))
    print()
    print('Use "xpider <command> -h" to see more info about a command')


def _print_unknown_command(cmdname):
    print("Unknown command: %s\n" % cmdname, file=sys.stderr)
    print('Use "xpider" to see available commands', file=sys.stderr)
...
def main(argv=None):
    ...
    parser.usage = "xpider {} {}".format(cmdname, cmd.syntax)
